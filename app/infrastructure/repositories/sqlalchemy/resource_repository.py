...
from app.domain.schemas.resource.resource import ResourceRead, ResourceCreate, ResourceUpdate
...
class SQLAlchemyResourceRepository(
    SQLAlchemyBaseRepository[ResourceRead, ResourceCreate, ResourceUpdate, BaseResource],
    IResourceRepository,
):
    """
    ...
    async def get(self, obj_id: int) -> Optional[ResourceRead]:
    ...
    async def list(self) -> List[ResourceRead]:
        resource_poly = with_polymorphic(BaseResource, [Rt, Station, CrawlerRoute, PandemicRoute])
    ...
    async def get_with_latest_state(self, obj_id: int) -> Optional[ResourceRead]:
        resource_poly = with_polymorphic(BaseResource, [Rt, Station, CrawlerRoute, PandemicRoute])
        ...
        stmt = (
            select(resource_poly)
            .join(latest_history_subq, BaseResource.id == latest_history_subq.c.resource_id, isouter=True)
            .options(contains_eager(resource_poly.state_history, alias=latest_history_subq))
    ...
    async def list_with_latest_state(self) -> List[ResourceRead]:
        resource_poly = with_polymorphic(BaseResource, [Rt, Station, CrawlerRoute, PandemicRoute])
        ...
        latest_history = (
            ...
            .join(
                latest_subq,
                (ResourceStateHistory.resource_id == latest_subq.c.resource_id)
                & (ResourceStateHistory.changed_at == latest_subq.c.max_changed_at)
            )
            .subquery()
        )
    ...
    async def list_by_type(self, resource_type: ResourceType) -> List[ResourceRead]:
        resource_poly = with_polymorphic(BaseResource, [Rt, Station, CrawlerRoute, PandemicRoute])
    ...
    async def create(self, create_schema: ResourceCreate) -> ResourceRead:
    ...
    async def update(self, obj_id: int, update_schema: ResourceUpdate) -> Optional[ResourceRead]:
